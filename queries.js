...
const createPerson = (request, response) => {
    const { name, firstname, birthdate, department } = request.body;

    pool.query('INSERT INTO personnel (name, firstname, birthdate, department) VALUES ($1, $2, $3, $4)', [name, firstname, birthdate, department], (error) => {
        if (error) {
            throw error
        }
...
const updatePerson = (request, response) => {
    ...
    pool.query('UPDATE personnel SET name = $1, firstname = $2 WHERE id = $3', [name, firstname, id], (error) => {
        if (error) {
            throw error
        }
        response.status(200).send(`User modified with ID: ${id}`)
...
const deletePerson = (request, response) => {
    ...
    pool.query('DELETE from personnel WHERE id = $1', [id], (error) => {
        if (error) {
            throw error
        }
        response.status(200).send(`User modified with ID: ${id}`)
